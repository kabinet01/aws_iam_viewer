...
import { IAMInlinePolicy, IAMPolicy, IAMRole, IAMGroup, ProcessedIAMData } from '@/lib/types';
import { collectInlinePolicies, formatDateTime, truncateArn } from '@/lib/iam-utils';
...
export default function DashboardPage() {
  ...
  const policyRiskMap = useMemo(() => {
    ...
    for (const policy of inlinePolicies) {
      ...
      if (matches.length > 0) riskMap[policy.id] = matches.length;
    }
  ...
  const filteredInlinePolicies = inlinePolicies.filter((policy) => {
    const term = searchTerm.toLowerCase();
    return (
      policy.PolicyName.toLowerCase().includes(term) ||
      policy.ownerName.toLowerCase().includes(term) ||
      policy.ownerArn.toLowerCase().includes(term) ||
      policy.ownerType.includes(term)
    );
  });
  ...
  return (
    <DashboardContent
      ...
      filteredInlinePolicies={filteredInlinePolicies}
      ...
      totalPolicies={Object.keys(policies).length + inlinePolicies.length}
...
type DashboardContentProps = {
  ...
  filteredInlinePolicies: IAMInlinePolicy[];
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      >
        ...
        <TabsContent value="policies" className="space-y-6">
          ...
          <InlinePolicyListCard
            policies={filteredInlinePolicies}
            total={inlinePolicies.length}
            policyRiskMap={policyRiskMap}
          />
          <PolicyListCard
    ...
    <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">{inlinePolicies} inline</p>
        </CardContent>
      </Card>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-y-0 pb-2">
...
function InlinePolicyListCard({
  ...
  return (
    <EntitiesCard title="Inline Policies" badge={total} count={policies.length} total={total}>
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>Policy Name</TableHead>
        ...
        <TableBody>
          {policies.map((policy) => {
            const riskCount = policyRiskMap[policy.id] || 0;
            return (
              <ClickableTableRow key={policy.id} href={`/${policy.ownerType}/${policy.ownerId}`}>
                <TableCell className="font-medium">{policy.PolicyName}</TableCell>
                <TableCell>
                  <Badge variant="secondary" className="capitalize">{policy.ownerType}</Badge>
                </TableCell>
                <TableCell>
                  <div>{policy.ownerName}</div>
                  <span className="font-mono text-xs text-muted-foreground">{truncateArn(policy.ownerArn)}</span>
                </TableCell>
                <TableCell>
                  {riskCount > 0 ? (
                    <Badge variant="destructive" className="text-xs">
